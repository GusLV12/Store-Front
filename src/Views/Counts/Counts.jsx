...
const { openModal, closeModal, } = useModal();
  ...
  const { makeRequest, response, loading } = useRequest(getUsers);
  const { makeRequest: tryDeleteProduct } = useRequest(deleteUser);
  ...
  const handleDelete = (id) => openModal(
    <DeleteModal
      ...
      id={id}
      descripcion="¿Seguro que deseas eliminar este usuario?"
        ...
        <Grid item xs={12}>
          ...
          <div className="flex flex-col justify-center">
            <ComposedTable
              ...
              isLoading={loading}
            >
              <ComposedTable.Column content={({ username }) => username} />
              <ComposedTable.Column content={({ email }) => email} />
              <ComposedTable.Column content={({ phone }) => phone} />
              <ComposedTable.Column content={({ role }) => role} />
